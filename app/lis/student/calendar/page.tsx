...
// Event color schemes
const EVENT_COLORS = {
  math: { color: "#1e3a8a", textColor: "#4A5F82", bgColor: "#dae4f2" },
...
export default function CalendarPage() {
  ...
  const getEventLink = (event: CalendarEvent) => {
    if (event.type === "class") {
      ...
      return `/lis/student/courses/${event.courseId}`;
    } else if (event.assignmentId && event.courseId) {
      return `/lis/student/courses/${event.courseId}/assignments/${event.assignmentId}`;
    }
    return "#";
  };
                    ...
                    <div className="space-y-1">
                      ...
                      {day.events.length > 4 && (
                        <button
                          ...
                          className="text-xs text-[#1e3a8a] hover:text-[#5a7399] dark:text-[#8fa8cc] dark:hover:text-[#1e3a8a] pl-1 cursor-pointer hover:underline"
                        >
